...
/**
 * URL de l'image actuellement rangée au `target`, sans getter par template :
 * on laisse `setImage` écrire un marqueur, et on relit la valeur qu'il a
 * remplacée. Toute nouvelle entrée du registre est ainsi couverte d'office.
 */
export function currentImageUrl(type: string, content: unknown, target: string): string {
  const setImage = TEMPLATE_UI[type]?.setImage;
  if (!setImage) return "";
  const marker = "__current-image-probe__";
  const replaced = (before: unknown, after: unknown): string | undefined => {
    if (after === marker) return typeof before === "string" ? before : "";
    if (!after || typeof after !== "object" || after === before) return undefined;
    for (const [key, value] of Object.entries(after)) {
      const found = replaced((before as Record<string, unknown> | undefined)?.[key], value);
      if (found !== undefined) return found;
    }
    return undefined;
  };
  return replaced(content, setImage(content, target, marker)) ?? "";
}
