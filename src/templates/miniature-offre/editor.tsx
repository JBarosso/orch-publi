...
interface MiniatureOffreEditorProps {
  items: MiniatureOffreItem[];
  briefWeek: number;
  onChange: (items: MiniatureOffreItem[]) => void;
  onOpenMediaLibrary: (itemId: string) => void;
  onDropFile?: (itemId: string, file: File) => void;
}

export function MiniatureOffreEditor({
  items,
  briefWeek,
  onChange,
  onOpenMediaLibrary,
  ...
  const addItem = () => {
    onChange([...items, createEmptyMiniatureOffreItem(uuidv4())]);
  };

  const updateItem = (id: string, updates: Partial<MiniatureOffreItem>) => {
    onChange(
      items.map((item) => {
    ...
    <div className="space-y-3">
      ...
      {items.length === 0 && (
        <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">
          Aucune miniature. Cliquez sur &quot;Ajouter&quot; pour commencer.
        </div>
      )}
    </div>
  );
