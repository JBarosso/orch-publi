...
export function EditoEditor({ items, briefWeek, onChange, onOpenMediaLibrary }: EditoEditorProps) {
  ...
  const addItem = () => {
    onChange([...items, createEmptyEditoCard(uuidv4())]);
  };

  const updateItem = (id: string, updates: Partial<EditoCard>) => {
    onChange(
      items.map((item) => {
        ...
        if ("imageWeek" in updates) {
          if (next.imageWeek != null && next.imageWeek !== briefWeek) {
            ...
            // actuelle (toutes les cartes edito comptent, pas de filtre
            // "visible" contrairement aux macarons/MEA).
    ...
    <div className="space-y-3">
      ...
      {items.length === 0 && (
        <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">
          Aucune carte edito. Cliquez sur &quot;Ajouter&quot; pour commencer.
        </div>
      )}
    </div>
  );
