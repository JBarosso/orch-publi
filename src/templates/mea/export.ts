...
function getButtonUrl(btn: MeaButton): string {
  ...
  if (btn.linkType === "cid")
    return `$url('Search-Show','cid','${esc(btn.cid.trim().replace(/\s/g, ""))}')$`;
...
export function generateMeaHTML(items: MeaItem[], ctx: ExportContext): string {
  ...
  const itemsHTML = visibleItems
    .map((item, index) => {
      ...
      const buttonsHTML = buttons
        .map(
          (btn) =>
            `                    <a class="mea__button" href="${getButtonUrl(btn)}">${esc(btn.text)}</a>`,
        )
        .join("\n");
...
export function generatePreviewHTML(items: MeaItem[]): string {
  const visibleItems = items.filter((item) => item.visible);
  ...
  const itemsHTML = visibleItems
    .map((item, index) => {
      ...
      const buttonsHTML = buttons
        .map(
          (btn) =>
            `                    <a class="mea__button" href="#">${esc(btn.text)}</a>`,
        )
        .join("\n");
